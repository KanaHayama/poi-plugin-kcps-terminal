...
+ 缩放
  ...
  返回画面相对于游戏默认画面大小(1200×720)的比例。传输前会按这个比例将截图缩放，因此影响画面质量的首先是浏览器自身的画面大小。这会影响传输带宽、判断的准确性。实际上是根据画面截图缩放的，这里以默认画面大小为标准的好处是在随意改变窗口大小时返回的图片分辨率也是固定的（实际上KCPS不要求返回的画面大小固定）。
+ JPEG质量
...
因为这个插件发布到npm上了，所以在poi“扩展程序”设置中输入包名*poi-plugin-kcps-terminal*就可以下载了（大概）。
