...
export const createRequestHandler = (deps: RouteDependencies) => {
  return (
    _req: http.IncomingMessage,
    res: http.ServerResponse,
    url: URL,
  ): void => {
    const storage = deps.eventHandler.getRequestStorage()
    const type = url.searchParams.get('type')?.trim()

    if (!type) {
      // Debug: return all stored requests sorted by key
